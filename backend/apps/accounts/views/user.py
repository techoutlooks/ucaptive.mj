...
from ..forms.user import NgLoginForm
...
from ..forms import NgLoginForm, NgRegistrationForm
...
class NgRegistrationView(NgFormViewMixin, FormView):
    """
    View to send email based on input received from an ajax call.
    Does not view however DRF3 API calls however (sent directly to APIView/Serializer).

    """
    form_class = NgRegistrationForm
    template_name = 'partials/auth.html'
    ...
    def post(self, request, **kwargs):
        ...
        if request.is_ajax():
            return self.ajax(request)
        return super(NgRegistrationView, self).post(request, **kwargs)
