...
class RadUserManager(models.Manager):
    def get_queryset(self):
        return RadUserQuerySet(self.model).filter(attribute=get_setting('DEFAULT_PASSWORD_ATTRIBUTE'), op=':=')

    def create(self, **kwargs):
        if 'password' in kwargs:
            kwargs['value'] = kwargs['password']
            del kwargs['password']
        kwargs.update(dict(attribute=get_setting('DEFAULT_PASSWORD_ATTRIBUTE'), op=':='))
    ...
    def get_or_create(self, **kwargs):
        ...
        #    kwargs['value'] = kwargs['password']
        #    del kwargs['password']
        kwargs['defaults'].update(dict(attribute=get_setting('DEFAULT_PASSWORD_ATTRIBUTE'), op=':='))
...
class RadGroup(object):
    """ Freeradius v3 radgroup table management."""
